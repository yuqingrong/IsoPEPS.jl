...
"""
    var_samples(g::Float64, J::Float64, row::Int; data_dir="data", save_path=nothing, sample_sizes=100:100:10000)

Calculate and plot variance of Hamiltonian energy as a function of sample size.

# Arguments
- `g::Float64`: Transverse field strength
- `J::Float64`: Coupling strength (default 1.0)
- `row::Int`: Number of rows in the lattice (for ZᵢZᵢ₊ᵣₒᵥ term)
- `data_dir::String`: Directory containing data files
- `save_path::Union{String,Nothing}`: Path to save the plot (nothing = auto-generate)
- `sample_sizes`: Range of sample sizes to analyze

# Description
Reads X and Z measurement data from the last line of data files and calculates
the Hamiltonian H = -g ∑ᵢ Xᵢ - J ∑ᵢ ZᵢZᵢ₊₁ - J ∑ᵢ ZᵢZᵢ₊ᵣₒᵥ for each sample.
Then computes variance as a function of sample size by dividing data into blocks.
"""
...
function ACF(g::Float64, row::Int; measure_first=:Z, data_dir="data", save_path=nothing, max_lag=nothing)
    
    filename = joinpath(data_dir, "$(measure_first)_first_$(row)_$(measure_first)_list_list_g=$(g).dat")
    ...
    try
        ...
        p = Plots.plot(
            ...
            xlabel="k",
            ylabel="⟨Z_j Z_(j+k)⟩",
            title="$(measure_first) correlation for g=$g, row=$row, τ_c_fit =$(round(xi_fit, digits=2))",
            legend=:topright,
            linewidth=2,
            size=(800, 600),
            marker=:circle,
        ...
        if fit_result !== nothing
            ...
            Plots.plot!(p, lags, fitted_curve, 
                ...
                linestyle=:dash, 
                color=:red,
                label="Fit: exp(-k/$(round(xi_fit, digits=2)))"
        )
        end
        ...
        # Save the plot
        if save_path === nothing
            save_path = "image/$(measure_first)_$(row)_ACF_g=$(g).pdf"
        end
        savefig(p, save_path)
        @info "Plot saved as '$save_path'"
...
"""
    energy_con(g::Union{Float64, Vector{Float64}}; data_dir="data")

Read energy data from energy_data_g=....dat and plot the energy variation over iterations.

# Arguments
- `g::Union{Float64, Vector{Float64}}`: Single transverse field parameter value or vector of values
- `data_dir::String`: Directory containing the data files (default: "data")

# Returns
- Plot object showing energy convergence

# Description
Reads energy data from the file(s) `energy_data_g=\$(g).dat` and creates a plot showing
how the energy varies over the optimization iterations. The data file should contain
one energy value per line. If a vector of g values is provided, all curves are plotted
in the same figure with different colors and a legend.
"""
function energy_con(g::Union{Float64, Vector{Float64}}; data_dir="data")
    # Convert single value to vector for uniform processing
    g_values = g isa Float64 ? [g] : g
    ...
    for (idx, g_val) in enumerate(g_values)
        # Construct filename
        filename = joinpath(data_dir, "energy_list_g=$(g_val).dat")
