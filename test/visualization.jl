...
function write_test_finite_analysis(path; J2, J1=1.0, row=2, Lx=4,
                                    neel=0.2, stripe_0pi=0.05,
                                    dimer_orientation=:vertical)
    qx = collect((0:(Lx - 1)) .* (2π / Lx))
    qy = collect((0:(row - 1)) .* (2π / row))
    S = [0.1 + 0.01 * ix + 0.02 * iy + J2 for ix in 1:Lx, iy in 1:row]
    M2 = S ./ (row * Lx)
    if iseven(row) && iseven(Lx)
        M2[div(Lx, 2) + 1, div(row, 2) + 1] = neel
        M2[1, div(row, 2) + 1] = stripe_0pi
    end
    SD = [0.03 * ix + 0.01 * iy + J2 / 10 for ix in 1:Lx, iy in 1:row]
    S_err = fill(0.01, Lx, row)
    M2_err = fill(0.005, Lx, row)
    SD_err = fill(0.002, Lx, row)
    columns(matrix) = [collect(column) for column in eachcol(matrix)]

    save_results(path;
        result_type="finite_cylinder_structure_factors",
        schema_version=1,
        source_samples_file="samples_J2=$(J2).json",
        metadata=Dict(
            "row" => row,
            "Lx" => Lx,
            "n_chains" => 4,
        ...
        qx_values=qx,
        qy_values=qy,
        matrix_layout="columns",
        spin_structure_factor=columns(S),
        magnetic_order_squared=columns(M2),
        dimer_structure_factor=columns(SD),
        spin_structure_factor_stderr=columns(S_err),
        magnetic_order_squared_stderr=columns(M2_err),
        dimer_structure_factor_stderr=columns(SD_err))
    return path
end
...
@testset "finite-cylinder J2 analysis discovery" begin
    data_dir = mktempdir()
    write_test_finite_analysis(
        joinpath(data_dir, "finite_analysis_J2=0.0.json");
        J2=0.0, neel=0.22, stripe_0pi=0.03)
    write_test_finite_analysis(
        joinpath(data_dir, "finite_analysis_J2=0.50.json");
        J2=0.5, neel=0.08, stripe_0pi=0.12)

    analyses = IsoPEPS._discover_finite_cylinder_analyses(
        data_dir, Float64[0.5, 0.25, 0.0];
        filename_template="finite_analysis_J2={J2}.json")
    @test getproperty.(analyses, :J2) == [0.0, 0.5]
    @test basename(analyses[2].path) == "finite_analysis_J2=0.50.json"

    duplicate_dir = mktempdir()
    write_test_finite_analysis(
        joinpath(duplicate_dir, "finite_analysis_J2=0.50.json"); J2=0.5)
    write_test_finite_analysis(
        joinpath(duplicate_dir, "finite_analysis_J2=0.500.json"); J2=0.5)
    @test_throws ArgumentError IsoPEPS._discover_finite_cylinder_analyses(
        duplicate_dir, Float64[0.5];
        filename_template="finite_analysis_J2={J2}.json")

    mismatch_dir = mktempdir()
    write_test_finite_analysis(
        joinpath(mismatch_dir, "finite_analysis_J2=0.5.json"); J2=0.6)
    @test_throws ArgumentError IsoPEPS._discover_finite_cylinder_analyses(
        mismatch_dir, Float64[0.5];
        filename_template="finite_analysis_J2={J2}.json")

    @test_throws ArgumentError IsoPEPS._discover_finite_cylinder_analyses(
        data_dir, Float64[0.0, 0.0];
        filename_template="finite_analysis_J2={J2}.json")
    @test_throws ArgumentError IsoPEPS._discover_finite_cylinder_analyses(
        data_dir, Float64[0.0];
        filename_template="finite_analysis.json")
end

@testset "finite-cylinder saved-analysis plots" begin
    data_dir = mktempdir()
    write_test_finite_analysis(
        joinpath(data_dir, "finite_analysis_J2=0.0.json");
        J2=0.0, neel=0.22, stripe_0pi=0.03)
    write_test_finite_analysis(
        joinpath(data_dir, "finite_analysis_J2=0.5.json");
        J2=0.5, neel=0.08, stripe_0pi=0.12)

    m2_path = joinpath(data_dir, "M2_finite.pdf")
    fig = plot_finite_cylinder_M2_comparison(
        data_dir, Float64[0.5, 0.0];
        show_errorbars=true,
        save_path=m2_path)
    @test fig isa Figure
    @test isfile(m2_path)
    axes = filter(content -> content isa Axis, fig.content)
    @test length(axes) == 2
    @test count(plot -> plot isa Errorbars, axes[2].scene.plots) == 2

    heatmap_path = joinpath(data_dir, "finite_heatmaps.pdf")
    heatmap_fig, spin_matrices, dimer_matrices =
        plot_finite_cylinder_combined_structure_factors(
            data_dir, Float64[0.5, 0.0];
            save_path=heatmap_path)
    @test heatmap_fig isa Figure
    @test isfile(heatmap_path)
    @test length(spin_matrices) == 2
    @test length(dimer_matrices) == 2
    @test size(spin_matrices[1]) == (4, 2)
    @test spin_matrices[1][1, 1] < spin_matrices[2][1, 1]
    periodic_matrices, qy_edges =
        IsoPEPS._finite_periodic_qy_display([spin_matrices[1]], 2)
    @test qy_edges ≈ [0.0, π / 2, 3π / 2, 2π]
    @test periodic_matrices[1] == hcat(spin_matrices[1], spin_matrices[1][:, 1])
    @test_throws ArgumentError plot_finite_cylinder_combined_structure_factors(
        data_dir, Float64[0.0]; color_scale=:invalid)
    @test_throws ArgumentError plot_finite_cylinder_combined_structure_factors(
        data_dir, Float64[0.0]; max_display_qx_bins=0)

    odd_dir = mktempdir()
    write_test_finite_analysis(
        joinpath(odd_dir, "finite_analysis_J2=0.0.json");
        J2=0.0, Lx=3)
    @test_throws ArgumentError plot_finite_cylinder_M2_comparison(
        odd_dir, Float64[0.0])

    incompatible_dir = mktempdir()
    write_test_finite_analysis(
        joinpath(incompatible_dir, "finite_analysis_J2=0.0.json");
        J2=0.0, row=2)
    write_test_finite_analysis(
        joinpath(incompatible_dir, "finite_analysis_J2=0.5.json");
        J2=0.5, row=4)
    @test_throws ArgumentError plot_finite_cylinder_combined_structure_factors(
        incompatible_dir, Float64[0.0, 0.5])
end
