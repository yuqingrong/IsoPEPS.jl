...
@testset "finite-cylinder FFT structure factors" begin
    row = 2
    Lx = 3
    N = row * Lx
    X = Float64[1, -1, 1, 1, -1, 1]
    Z = Float64[1, 1, -1, 1, 1, -1]
    Y = Float64[-1, 1, 1, -1, 1, 1]

    function direct_spin_structure_factor(X, Z, Y, row)
        Lx = length(X) ÷ row
        N = row * Lx
        S = zeros(Float64, Lx, row)
        idx(pos, col) = row * (col - 1) + pos
        for nx in 0:(Lx - 1), ny in 0:(row - 1)
            qx = 2π * nx / Lx
            qy = 2π * ny / row
            total = 0.0
            for c1 in 1:Lx, p1 in 1:row, c2 in 1:Lx, p2 in 1:row
                i = idx(p1, c1)
                j = idx(p2, c2)
                phase = cos(qx * (c2 - c1) + qy * (p2 - p1))
                total += phase * (X[i] * X[j] + Y[i] * Y[j] + Z[i] * Z[j]) / 4
            end
            S[nx + 1, ny + 1] = total / N
        end
        return S
    ...
    S_fft = finite_cylinder_spin_structure_factor(X, Z, Y, row)
    @test S_fft ≈ direct_spin_structure_factor(X, Z, Y, row)
    @test finite_cylinder_magnetic_order_squared(X, Z, Y, row) ≈ S_fft ./ N

    ones_samples = ones(row * Lx)
    S_ones = finite_cylinder_spin_structure_factor(ones_samples, ones_samples, ones_samples, row)
    M2_ones = finite_cylinder_magnetic_order_squared(ones_samples, ones_samples, ones_samples, row)
    @test S_ones[1, 1] ≈ 3N / 4
    @test M2_ones[1, 1] ≈ 3 / 4
    nonzero_modes = collect(vec(S_ones))
    deleteat!(nonzero_modes, 1)
    @test maximum(abs.(nonzero_modes)) < 1e-12
    @test maximum(abs.(finite_cylinder_dimer_structure_factor(
        ones_samples, ones_samples, ones_samples, row))) < 1e-12

    row_af = 2
    Lx_af = 4
    af = [(-1.0)^(pos + col) for col in 1:Lx_af for pos in 1:row_af]
    S_af = finite_cylinder_spin_structure_factor(af, af, af, row_af)
    @test argmax(S_af) == CartesianIndex(3, 2)  # q = (π, π)
end

@testset "finite-cylinder saved samples analysis" begin
    row = 2
    Lx = 3
    X = Float64[1, -1, 1, 1, -1, 1]
    Z = Float64[1, 1, -1, 1, 1, -1]
    Y = Float64[-1, 1, 1, -1, 1, 1]
    sample_file = tempname() * ".json"
    analysis_file = tempname() * ".json"

    save_results(sample_file;
    ...
    qx, qy, S, M2, SD, S_err, M2_err, SD_err =
        finite_cylinder_structure_factors_from_samples(sample_file;
            save_path=analysis_file)
    @test qx ≈ collect((0:(Lx - 1)) .* (2π / Lx))
    @test qy ≈ collect((0:(row - 1)) .* (2π / row))
    @test S ≈ finite_cylinder_spin_structure_factor(X, Z, Y, row)
    @test M2 ≈ S ./ (row * Lx)
    @test size(SD) == (Lx, row)
    @test maximum(abs.(S_err)) < 1e-12
    @test maximum(abs.(M2_err)) < 1e-12
    @test maximum(abs.(SD_err)) < 1e-12
    @test isfile(analysis_file)
    saved = load_results(analysis_file)
    @test haskey(saved, "spin_structure_factor")
    @test haskey(saved, "magnetic_order_squared")
    @test haskey(saved, "dimer_structure_factor")
    @test saved["result_type"] == "finite_cylinder_structure_factors"
    @test saved["schema_version"] == 1
    @test saved["metadata"]["J1"] == 1.0
    @test saved["metadata"]["J2"] == 0.5
    @test_throws UndefKeywordError finite_cylinder_structure_factors_from_samples(sample_file)
end

@testset "save finite-cylinder samples smoke" begin
    data_dir = mktempdir()
    row = 1
    p = 1
    nqubits = 1
    ...
    input_args = Dict{Symbol,Any}(
        :model => "tfim", :J => 1.0, :g => 1.0,
        :row => row, :p => p, :nqubits => nqubits,
        :share_params => true,
    )
    optimized_file = joinpath(data_dir, "tiny_circuit.json")
    samples_file = joinpath(data_dir, "finite_samples.json")
    save_result(optimized_file, result, input_args)

    saved_result = save_finite_cylinder_samples(optimized_file, samples_file;
        Lx=Lx, n_chains=1, conv_step=0, measure_y=true)
    @test saved_result.row == row
    @test saved_result.Lx == Lx
    saved = load_results(samples_file)
    @test saved["sample_layout"] == "chains_flat_column_major"
    @test saved["measure_y"] == true
    @test length(saved["X_samples"]) == 1
    @test length(saved["X_samples"][1]) == row * Lx
    @test length(saved["Y_samples"][1]) == row * Lx
end
